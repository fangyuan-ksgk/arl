"""
GRPO on Wordle hacking-game environment

Single-GPU:
    python scripts/grpo_wordle.py                                    # 16 episodes, 1 epoch
    python scripts/grpo_wordle.py --n_episodes 64 --num_train_epochs 3

Multi-GPU:
    accelerate launch --num_processes 4 scripts/grpo_wordle.py --n_episodes 64
    accelerate launch --config_file scripts/configs/multi_gpu.yaml scripts/grpo_wordle.py

DeepSpeed ZeRO-2:
    accelerate launch --config_file scripts/configs/deepspeed_zero2.yaml scripts/grpo_wordle.py
"""
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="GRPO on Wordle")
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-0.6B")
    parser.add_argument("--oracle_model", type=str, default="claude-sonnet-4-20250514")
    parser.add_argument("--output_dir", type=str, default="grpo_wordle_output")
    ...
    if state.is_main_process:
        ...
        train_dataset.save_to_disk("/tmp/_wordle_grpo_dataset")
        rollout_model.cpu()
        torch.cuda.empty_cache()
    state.wait_for_everyone()
    if not state.is_main_process:
        from datasets import load_from_disk
        train_dataset = load_from_disk("/tmp/_wordle_grpo_dataset")
